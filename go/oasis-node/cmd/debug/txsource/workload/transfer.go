...
func (t *transfer) doTransferTx(ctx context.Context, fromIdx int, toIdx int) error {
	...
	if err := transfer.BaseUnits.FromInt64(transferAmount); err != nil {
	...
	if err := from.reckonedBalance.Sub(&transfer.BaseUnits); err != nil {
		return fmt.Errorf("from reckoned balance %v Sub transfer base units %v: %w",
			from.reckonedBalance, transfer.BaseUnits, err,
		)
	}
	if err := to.reckonedBalance.Add(&transfer.BaseUnits); err != nil {
		return fmt.Errorf("to reckoned balance %v Add transfer base units %v: %w",
			to.reckonedBalance, transfer.BaseUnits, err,
		)
	}
...
func (t *transfer) doBurnTx(ctx context.Context, idx int) error {
	...
	burn := staking.Burn{}
	if err := burn.BaseUnits.FromInt64(transferBurnAmount); err != nil {
