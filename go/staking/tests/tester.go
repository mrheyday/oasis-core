...
func testTransfer(t *testing.T, state *stakingTestsState, backend api.Backend, consensus consensusAPI.Backend) {
	...
	xfer := &api.Transfer{
		To:        DestAddr,
		BaseUnits: debug.QtyFromInt(math.MaxUint8),
	}
...
TransferWaitLoop:
	for {
		...
		case ev := <-ch:
			if ev.From.Equal(api.CommonPoolAddress) || ev.To.Equal(api.CommonPoolAddress) {
				require.False(ev.BaseUnits.IsZero(), "CommonPool xfer: amount should be non-zero")
				continue
			}
			if ev.From.Equal(api.FeeAccumulatorAddress) || ev.To.Equal(api.FeeAccumulatorAddress) {
				require.False(ev.BaseUnits.IsZero(), "FeeAccumulator xfer: amount should be non-zero")
				continue
			}
			...
			if !gotTransfer {
				...
				require.Equal(xfer.BaseUnits, ev.BaseUnits, "Event: base_units")
				...
				for _, evt := range evts {
					if evt.Transfer != nil {
						if evt.Transfer.From.Equal(ev.From) && evt.Transfer.To.Equal(ev.To) && evt.Transfer.BaseUnits.Cmp(&ev.BaseUnits) == 0 {
							gotTransfer = true
	...
	_ = srcAcc.General.Balance.Sub(&xfer.BaseUnits)
	...
	_ = dstAcc.General.Balance.Add(&xfer.BaseUnits)
	...
	xfer.BaseUnits = newSrcAcc.General.Balance
...
TransferWaitLoop:
	for {
		...
		case ev := <-ch:
			...
			to := ev.Transfer.To
			baseUnits := ev.Transfer.BaseUnits
			...
			if !gotTransfer {
				...
				require.Equal(xfer.BaseUnits, ev.Transfer.BaseUnits, "Event: base_units")
...
func testBurn(t *testing.T, state *stakingTestsState, backend api.Backend, consensus consensusAPI.Backend) {
	...
	burn := &api.Burn{
		BaseUnits: debug.QtyFromInt(math.MaxUint32),
	}
	...
	case ev := <-ch:
		...
		require.Equal(burn.BaseUnits, ev.BaseUnits, "Event: base_units")
		...
		for _, evt := range evts {
			if evt.Burn != nil {
				if evt.Burn.Owner.Equal(ev.Owner) && evt.Burn.BaseUnits.Cmp(&ev.BaseUnits) == 0 {
					gotIt = true
					break
				}
			}
	...
	_ = srcAcc.General.Balance.Sub(&burn.BaseUnits)
	newSrcAcc, err := backend.Account(context.Background(), &api.OwnerQuery{Owner: SrcAddr, Height: consensusAPI.HeightLatest})
	require.NoError(err, "src: Account")
	require.Equal(srcAcc.General.Balance, newSrcAcc.General.Balance, "src: general balance - after")
...
func testEscrowEx( // nolint: gocyclo
	...
	require.NoError(totalEscrowed.Add(&escrow.BaseUnits))
	...
	case rawEv := <-ch:
		...
		require.Equal(escrow.BaseUnits, ev.BaseUnits, "Event: base_units")
		...
		for _, evt := range evts {
			if evt.Escrow != nil && evt.Escrow.Add != nil {
				if evt.Escrow.Add.Owner.Equal(ev.Owner) && evt.Escrow.Add.Escrow.Equal(ev.Escrow) && evt.Escrow.Add.BaseUnits.Cmp(&ev.BaseUnits) == 0 {
					gotIt = true
					break
				}
			}
	...
	_ = dstAcc.Escrow.Active.Deposit(currentTotalShares, &srcAcc.General.Balance, &escrow.BaseUnits)
	...
	case rawEv := <-ch:
		...
		require.Equal(srcAddr, ev.Owner, "Event: owner")
		require.Equal(dstAddr, ev.Escrow, "Event: escrow")
		require.Equal(totalEscrowed, &ev.BaseUnits, "Event: base_units")
		...
		for _, evt := range evts {
			if evt.Escrow != nil && evt.Escrow.Reclaim != nil {
				if evt.Escrow.Reclaim.Owner.Equal(ev.Owner) && evt.Escrow.Reclaim.Escrow.Equal(ev.Escrow) && evt.Escrow.Reclaim.BaseUnits.Cmp(&ev.BaseUnits) == 0 {
					gotIt = true
					break
				}
			}
	...
	escrow = &api.Escrow{
		Account:   dstAddr,
		BaseUnits: debug.QtyFromInt(1), // Minimum is 10.
	}
	tx = api.NewAddEscrowTx(srcAcc.General.Nonce, nil, escrow)
	err = consensusAPI.SignAndSubmitTx(context.Background(), consensus, srcSigner, tx)
	...
	for {
		...
		case ev := <-slashCh:
			if e := ev.Take; e != nil {
				...
				// All base units must be slashed as defined in debugGenesisState.
				require.Equal(escrow.BaseUnits, e.BaseUnits, "TakeEscrowEvent - all base units slashed")
				break WaitLoop
			}
		case <-time.After(recvTimeout):
